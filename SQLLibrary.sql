	...
	IF (SELECT COUNT(*) FROM  INSERTED) > 0
		BEGIN
			...
			FETCH NEXT FROM CUR_BORROW_BOOK INTO @SERIAL
				...
				BEGIN
					SET @EVENT = CONCAT('CREATE BORROW_CARD ID : ', (SELECT ID FROM INSERTED ), ' - STUDENT :  ', (SELECT ID_STUDENT FROM INSERTED), ' - BOOK ID :  ', (SELECT @SERIAL))
					EXEC PROC_INSERT_LOG_EVENT @EVENT
					FETCH NEXT FROM CUR_BORROW_BOOK INTO @SERIAL
				END
	...
	ELSE
		IF (SELECT COUNT(*) FROM  DELETED) > 0
			BEGIN
				DECLARE CUR_BORROW_BOOK CURSOR
				FOR SELECT SERIAL 
						 FROM DELETED, BORROW_DETAIL
						 WHERE DELETED.ID = BORROW_DETAIL.ID
				OPEN CUR_BORROW_BOOK
				FETCH NEXT FROM CUR_BORROW_BOOK INTO @SERIAL
				WHILE (@@FETCH_STATUS = 0)
				BEGIN
					SET @EVENT = CONCAT('DELETE BORROW_CARD ID : ', (SELECT ID FROM INSERTED ), ' - STUDENT :  ', (SELECT ID_STUDENT FROM INSERTED), ' - BOOK ID :  ', (SELECT @SERIAL))
					EXEC PROC_INSERT_LOG_EVENT @EVENT
					FETCH NEXT FROM CUR_BORROW_BOOK INTO @SERIAL
				END
				CLOSE CUR_BORROW_BOOK
